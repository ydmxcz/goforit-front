                    ...
                    <PageHeader back hidden-breadcrumb>
                        ...
                        <template #title>
                            <div style="width: 100%;display: inline;">
                                <span>{{ problemId }}.{{ data.problem.info.title }}</span>
...
const problemIndex = ref(router.currentRoute.value.params.problemIdx);
...
const getStartTime = async () => {
    const { data: res } = await http.post("/contest/starttime", {
        id: BigNumber(contestId.value),
    })
    if (res.code != 200) {
        msg.err(res.msg)
        return
    ...
    if(date < res.data.startTime) {
        msg.err('比赛未开始，无法提交')
        return false
    } else if (date >= res.data.endTime){
        msg.err('比赛已结束，无法提交')
        return false
    }
    return true
...
const handleSubmitProblem = async () => {
    ...
    let d = {
        ...
        problemIdx:BigNumber(problemIndex.value),
        contestId:BigNumber(contestId.value)
    }
    console.log(d);
    const { data: res } = await http.post('/contest/submit', d)
    console.log(res);
    if (res.code != 200) {
        msg.err(res.msg)
        data.runRes = {
...
const getProblemInfo = async () => {
    const { data: res } = await http.post('/contest/problem/detial' ,{
        contestId:BigNumber(contestId.value),
        problemId:BigNumber(problemId.value)
    })
    if (res.code != 200) {
    ...
    data.problem.info = res.data.problem
